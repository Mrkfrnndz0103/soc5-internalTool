...
export default function LoginPage() {  
  ...
  useEffect(() => {  
    if (!isAuthenticated) {  
      const newSessionId = `seatalk-${Date.now()}-${Math.random()  
        .toString(36)  
        .substring(2, 11)}`;  
      ...
      setQrCode(  
        `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(  
          `seatalk://auth/soc5-outbound?session=${newSessionId}`,  
        )}`,  
      );  
  ...
  const handleSendHelp = () => {  
    if (helpMessage.trim()) {  
      toast({  
        title: "Message Sent",  
        description:  
          "Your message has been sent to the Data Team. They will respond shortly.",  
      });  
      setHelpMessage("");  
      setShowHelp(false);  
    }  
                  ...
                  >  
                    ...
                    {qrCode ? (  
                      <img  
                        src={qrCode}  
                        alt="QR Code"  
                        className="w-40 h-40 relative z-10 rounded-lg animate-in zoom-in-50 duration-500 delay-500"  
                      />  
                    ) : (  
              ...
              <div className="flex items-center justify-center relative animate-in fade-in duration-800 delay-200 -ml-4">  
                ...
                <div className="absolute animate-in zoom-in-50 duration-500 delay-700">  
                  <div className="relative">  
                    ...
                    <span className="relative bg-[#252836] px-2 py-0.5 text-white font-medium rounded-full border border-blue-400 text-[11px] shadow-lg">  
                      OR  
                    </span>  
                  </div>  
                </div>  
